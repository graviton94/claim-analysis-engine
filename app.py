# ============================================================================
# 메인 애플리케이션 엔트리
# ============================================================================
# Streamlit 앱 실행 설정 및 메인 페이지

import streamlit as st
from pathlib import Path

# ============================================================================
# 페이지 레이아웃 설정
# ============================================================================
st.set_page_config(
    page_title="Advanced Claim Prediction System",
    page_icon="🏆",
    layout="wide",
    initial_sidebar_state="expanded"
)

# ============================================================================
# 메인 페이지 콘텐츠
# ============================================================================
st.title("🏆 Advanced Claim Prediction System")
st.markdown(
    """
    ### Global ML/DL 챔피언 모델 기반, 플랜트 중심 정밀 분석 시스템
    
    단순 통계나 EWS를 넘어, Optuna로 튜닝된 최적의 모델(CatBoost, LSTM, SARIMAX)을 경쟁시켜 
    미래 리스크를 예측합니다.
    """
)

st.divider()

# ============================================================================
# 주요 기능 설명
# ============================================================================
col1, col2 = st.columns(2)

with col1:
    st.subheader("🎯 주요 목표")
    st.write(
        """
        1. **Hyper-Model**: ML/DL 모델 자동 튜닝 및 성능 비교
        2. **Plant-Centric**: 플랜트 기준 유연한 피벗 대시보드
        3. **Sales Integration**: 매출 데이터 통합 관리 및 PPM 분석
        4. **Robust ETL**: 54개 핵심 필드 자동 추출 및 연/월 파티셔닝
        """
    )

with col2:
    st.subheader("🛠 기술 스택")
    st.write(
        """
        - **ML/DL**: CatBoost, PyTorch (LSTM), Statsmodels (SARIMAX)
        - **AutoML**: Optuna (하이퍼파라미터 튜닝)
        - **Data**: Pandas, Pyarrow (Parquet Partitioning)
        - **UI**: Streamlit (Data Editor, Dynamic Pivot)
        - **Viz**: Plotly Express / Graph Objects
        """
    )

st.divider()

# ============================================================================
# 사이드바: 네비게이션
# ============================================================================
with st.sidebar:
    st.header("📍 네비게이션")
    
    st.subheader("**Phase 1: Data Foundation** ✅")
    st.write("완료 - 데이터 ETL & 매출 기본 관리")
    st.markdown(
        """
        - `1_데이터_업로드`: 클레임 데이터 파티셔닝 적재
        - `2_매출수량_관리`: 매출 데이터 CRUD
        """
    )
    
    st.divider()
    
    st.subheader("**Phase 2: Adaptive PPM Dashboard** ✅")
    st.write("완료 - 동적 피벗 & 스마트 매출 추정")
    st.markdown(
        """
        - `3_플랜트_분석`: 동적 피벗 + PPM 계산 + 시각화
        """
    )
    
    st.divider()
    
    st.subheader("**Phase 3: ML/DL Engine** (준비중)")
    st.markdown(
        """
        - `4_예측_시뮬레이션`: Optuna 튜닝 & 챔피언 모델
        """
    )
    
    st.divider()
    
    st.markdown(
        """
        ---
        **프로젝트 설정**
        - 🌿 Branch: `main`
        - 📂 Data: `data/hub/` (파티셔닝)
        - 💰 Sales: `data/sales/` (Smart Sync)
        - 📊 Models: `data/models/`
        """
    )

st.divider()

# ============================================================================
# 시작 가이드
# ============================================================================
st.subheader("📚 빠른 시작 가이드")

with st.expander("1️⃣ Step 1: 데이터 업로드 (필수)", expanded=True):
    st.markdown(
        """
        1. 좌측 네비게이션에서 **`1_데이터_업로드`** 페이지로 이동
        2. CSV 또는 Excel 파일 선택
        3. 자동 처리 (54개 필드 추출)
        4. 파티셔닝 저장 (접수년/접수월 기준)
        
        ✅ 처리된 데이터는 `data/hub/접수년=YYYY/접수월=MM/` 폴더에 저장됩니다.
        """
    )

with st.expander("2️⃣ Step 2: 매출 데이터 입력 (권장)", expanded=True):
    st.markdown(
        """
        1. 좌측 네비게이션에서 **`2_매출수량_관리`** 페이지로 이동
        2. 플랜트별 년/월 매출수량을 엑셀 형식으로 입력
        3. 저장 (분석 시 클레임 데이터와 자동 연결)
        
        💡 매출 데이터를 제공하면 PPM(Parts Per Million) 지표를 계산할 수 있습니다.
        """
    )

with st.expander("3️⃣ Step 3: 피벗 분석 (완료)", expanded=False):
    st.markdown(
        """
        1. 좌측 네비게이션에서 **`3_플랜트_분석`** 페이지로 이동
        2. 분석할 플랜트 선택 (필수)
        3. 기간 필터 설정 (선택)
        4. 피벗 열 선택 (multiselect)
        5. 지표 선택 (건수 / PPM)
        6. 동적 피벗 테이블 & 시계열 차트 조회
        
        💡 PPM = (클레임건수 / 매출수량) × 1,000,000 자동 계산
        ⚠️ 매출이 없는 행은 직전 3개월 평균값 자동 추정
        """
    )

with st.expander("4️⃣ Step 4: 예측 시뮬레이션 (준비중)", expanded=False):
    st.markdown(
        """
        - CatBoost, LSTM, SARIMAX 모델 자동 학습
        - Optuna 하이퍼파라미터 최적화
        - 챔피언 모델 선정 및 6개월 예측
        """
    )

st.divider()

# ============================================================================
# 프로젝트 정보
# ============================================================================
st.subheader("ℹ️ 프로젝트 정보")

col_info1, col_info2 = st.columns(2)

with col_info1:
    st.write("**GitHub Repository**")
    st.write("[graviton94/claim-analysis-engine](https://github.com/graviton94/claim-analysis-engine/tree/main)")
    
with col_info2:
    st.write("**Current Phase**")
    st.write("Phase 2: Adaptive PPM Dashboard ✅")

st.write("")
st.caption("© 2026 Advanced Claim Prediction System | Branch: main")
